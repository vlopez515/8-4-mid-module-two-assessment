...
function getAllMoviesReleasedAtOrBeforeYear(movies, year) {
  ...
  return movies.filter(movie => movie.released.substr(movie.released.length-4) <= year)
}
...
function getRottenTomatoesScoreByMovie(movies) {
  if (movies.length === 0) {
   ...
   return movies.map(movie => {
     return { [movie.title] : movie.ratings.find(ratings => ratings.source === 'Rotten Tomatoes').value };
  });
}
